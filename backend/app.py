...
def create_app(overrides=None):
    ...
    def secure_response(response):
        ...
        if origin in app.config['ALLOWED_ORIGINS']:
            ...
            # The editor uses cookies when the frontend calls the API subdomain.
            # Credentials are accepted only for these exact, allowed origins.
            if request.path.startswith('/api/editor/'):
